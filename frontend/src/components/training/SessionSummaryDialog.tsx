...
type ReportTab = 'overview' | 'logs'

interface SessionSummaryDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  summary: StoredSessionReview | null
...
export function SessionSummaryDialog({ open, onOpenChange, summary, history = [] }: SessionSummaryDialogProps) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      {open && summary ? <SessionSummaryDialogContent key={summary.id} summary={summary} history={history} /> : null}
    </Dialog>
  )
}

function SessionSummaryDialogContent({ summary, history }: { summary: StoredSessionReview; history: StoredSessionReview[] }) {
  ...
  const visibleHistory = useMemo(() => history.filter((item) => item.id !== summary.id), [history, summary.id])
  const totalPages = Math.max(1, Math.ceil((selectedReview.logs?.length ?? 0) / LOG_PAGE_SIZE))
  const safeLogPage = Math.min(logPage, totalPages)
  const pagedLogs = useMemo(() => {
    const start = (safeLogPage - 1) * LOG_PAGE_SIZE
    return (selectedReview.logs ?? []).slice(start, start + LOG_PAGE_SIZE)
  }, [safeLogPage, selectedReview])
  ...
  return (
    ...
    >
      <DialogHeader>
        ...
        <DialogDescription>
          顶部保留摘要和 Top 项，右侧可切换最近训练；支持查看概览与逐题记录，方便复盘每一组表现。
        </DialogDescription>
      </DialogHeader>
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(280px,340px)] xl:grid-cols-[minmax(0,1fr)_320px]">
        <div className="space-y-4">
          <div className="rounded-2xl border bg-muted/25 p-4">
            <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground sm:text-sm">
              ...
              {selectedReview.id === summary.id ? (
                <Badge className="bg-emerald-500/10 text-emerald-700 dark:text-emerald-300">当前查看</Badge>
              ) : null}
            </div>
          </div>
          ...
          <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as ReportTab)}>
            <TabsList>
              ...
              <TabsTrigger value="logs">逐题记录</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="overview" className="space-y-4">
              ...
              <div className="rounded-2xl border">
                <div className="flex items-center justify-between gap-3 border-b px-4 py-3">
                  ...
                  <Badge className="bg-secondary text-secondary-foreground">{selectedReview.breakdown?.length ?? 0} 项</Badge>
                </div>
            ...
            <TabsContent value="logs" className="space-y-4">
              <div className="flex items-center justify-end gap-2">
                <Button variant="outline" size="sm" onClick={() => setLogPage(Math.max(1, safeLogPage - 1))} disabled={safeLogPage <= 1}>
                  <ChevronLeft className="h-4 w-4" />
                  上一页
                </Button>
                <Button variant="outline" size="sm" onClick={() => setLogPage(Math.min(totalPages, safeLogPage + 1))} disabled={safeLogPage >= totalPages}>
                  下一页
              ...
              <LogTable
                ...
                heading="逐题记录（最新在上）"
                pageHint="会保留这一组的逐题详情；切换到其他历史报告时，页码会自动重置为第一页。"
                emptyMessage="这一组还没有逐题记录，可能是旧数据，或当前样本不足。"
              />
            </TabsContent>
          </Tabs>
        </div>
        ...
        <div className="rounded-2xl border bg-muted/20 p-4">
          ...
          <div className="mt-4 space-y-3">
            <Button variant={selectedReview.id === summary.id ? 'default' : 'outline'} className="w-full justify-start" onClick={() => selectReview(summary.id)}>
