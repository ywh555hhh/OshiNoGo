...
function session(
  ...
  return { startedAt, endedAt: startedAt + 60_000, durationMs: 60_000, channel, events }
}

function archive(sessions: ArchivedSession[]): Archive {
  return { version: 2, drillId: 'kana', exportedAt: 0, sessions }
}
...
describe('buildItemHistory', () => {
  ...
  it('lastSeenAt 取 session 的墙钟时间，而不是事件里的 tOnset（那是单调时钟）', () => {
    const history = buildItemHistory(archive([session(NOW - DAY, [event('a', 400, true)])]), 'tap')

    // 事件 tOnset 是 1000 量级（performance.now 风格），绝不能被当成时间
  ...
  it('不测速度的通道（graduateRt: null）只按准确率毕业', () => {
    const history = buildItemHistory(
  ...
  it('准确率不够就不毕业，哪怕很快', () => {
    ...
    const history = buildItemHistory(archive([session(NOW, events)]), 'tap')
  ...
  it('只看指定通道', () => {
    const history = buildItemHistory(
      archive([
        session(NOW, [event('a', 400, true, 'tap')], 'tap'),
        session(NOW, [event('b', 400, true, 'type')], 'type'),
      ]),
      'tap',
    )

    expect(history.byItem.has('a')).toBe(true)
    expect(history.byItem.has('b')).toBe(false)
    expect(history.sessions).toBe(1)
  })

  it('空档案不炸', () => {
    const history = buildItemHistory(archive([]), 'tap')
...
describe('computeLifetime', () => {
  ...
  it('累计组数、题数、正确率', () => {
    const lifetime = computeLifetime(threeDays, 'tap', dayKey, NOW)
  ...
  it('性能时钟造成的时间戳不会被算成「同一天」', () => {
    // v2 档案里 startedAt 存过 performance.now()（页面加载至今的毫秒数）
    const bogus = archive([
      { startedAt: 1234, endedAt: 5678, durationMs: 4444, channel: 'tap', events: [event('a', 400, true)] },
      { startedAt: 9999, endedAt: 12_000, durationMs: 2001, channel: 'tap', events: [event('b', 400, true)] },
    ])

    const lifetime = computeLifetime(bogus, 'tap', dayKey, NOW)
  ...
  it('空档案给出零值而不是 NaN', () => {
    const lifetime = computeLifetime(archive([]), 'tap', dayKey, NOW)
